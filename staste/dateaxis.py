...
DATE_SCALES_RRULE_KWARGS = {
    'year': {'freq': rrule.YEARLY, 'bymonthday': 1},
    'month': {'freq': rrule.MONTHLY, 'bymonthday': 1},
...
DATE_SCALES_SCALE = {
    'year': lambda dt: datetime.datetime(dt.year, 1, 1),
    'month': lambda dt: datetime.datetime(dt.year, dt.month, 1),
    'day': lambda dt: datetime.datetime(year=dt.year, month=dt.month, day=dt.day),
    'hour': lambda dt: datetime.datetime(year=dt.year, month=dt.month, day=dt.day, hour=dt.hour),
    'minute': lambda dt: datetime.datetime(year=dt.year, month=dt.month, day=dt.day, hour=dt.hour, minute=dt.minute)
}
...
class DateAxis(object):
    ...
    def iterate(self, mv):
        """
        Iterates on all date values for scale of MetricaValues.
        Yields a number and an id (string part of the hash key)

        For example, if MetricaValues is filtered by month (like .timespan(month=2)),
        this will iterate by month days.
        """
        ...
        try:
            ...
            raise ValueError("Can't iterate further than %s" % DATE_SCALES_AND_EXPIRATIONS[-1][0])

        scale = scale_[0]

        for val in self.get_scale_range(scale, mv):
            yield int(val), ':'.join(id_parts + self._datepart_format(scale, val))
    ...
    def scale_timeserie(self, scale, since, until):
        rr = rrule.rrule(dtstart=since, until=until, **DATE_SCALES_RRULE_KWARGS[scale])
    ...
    def _datetime_to_id_parts(self, max_scale, dt):
        ...
        for scale, scale_expiration in DATE_SCALES_AND_EXPIRATIONS:
            ...
            id_parts += self._datepart_format(scale, val)
    ...
    def _datepart_format(self, scale, val):
        return [scale, str(val)]
        # return ["{}{}".format(scale, val)]
